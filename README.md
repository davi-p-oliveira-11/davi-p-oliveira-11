<h1 align="center">⚡ Davi P. Oliveira</h1>
<p align="center"><i>aka <a href="https://github.com/DavidWaldstein11" target="_blank">David Waldstein</a></i></p>

<p align="center">
  Full-Stack Developer specialized in building <b>production-ready web and mobile applications</b><br/>
  React • Angular • Node.js • Spring Boot • PostgreSQL • Cloud-Native
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/davi-p-oliveira-a44705218/">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-blue?style=for-the-badge&logo=linkedin"/>
  </a>
  <a href="https://www.instagram.com/davipoliveira11.dev/">
    <img src="https://img.shields.io/badge/Instagram-Follow-E4405F?style=for-the-badge&logo=instagram"/>
  </a>
</p>

---

## 🚀 Current Mission — Thunder Attack ⚡

Building a portfolio of **production-minded MVPs** to demonstrate real-world skills:

- 3x PERN Stack SaaS Projects (PostgreSQL, Express, React, Node)
- 6x Mobile Apps (React Native)
- 3x Enterprise Systems (Angular + Spring Boot)

Each project includes:
- Authentication & Authorization
- Clean Architecture
- API Documentation
- Dockerized Setup
- Deployed Demos
- Professional README & Screenshots

---

## 🧩 Featured Projects

| Project | Stack | Description |
|--------|-------|-------------|
| **Scholaryx** | PERN | Academic Management SaaS (Auth, Roles, Dashboard, REST API) |
| **Voltaris Landing Kit** | React + Tailwind | High-conversion landing pages for freelancers & startups |
| **Mobile Finance Tracker** | React Native | Offline-first personal finance app |

*(More coming as Thunder Attack progresses)*

---

## 🛠 Core Stack

### ⚛️ Frontend
<p align="center">
  <img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react" />
  <img src="https://img.shields.io/badge/Next.js-black?style=for-the-badge&logo=nextdotjs" />
  <img src="https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular" />
  <img src="https://img.shields.io/badge/Tailwind-06B6D4?style=for-the-badge&logo=tailwindcss" />
  <img src="https://img.shields.io/badge/shadcn%2Fui-111111?style=for-the-badge" />
  <img src="https://img.shields.io/badge/DaisyUI-5A0EF8?style=for-the-badge&logo=daisyui" />
</p>

### 🎨 Motion & 3D

<p align="center">
  <img src="https://img.shields.io/badge/GSAP-88CE02?style=for-the-badge&logo=greensock" />
  <img src="https://img.shields.io/badge/Three.js-black?style=for-the-badge&logo=three.js" />
</p>

### 🧠 Backend
<p align="center">
  <img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js" />
  <img src="https://img.shields.io/badge/Express-black?style=for-the-badge&logo=express" />
  <img src="https://img.shields.io/badge/Fastify-black?style=for-the-badge&logo=fastify" />
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=springboot" />
</p>

### 🗄 Data & Cloud
<p align="center">
  <img src="https://img.shields.io/badge/PostgreSQL-336791?style=for-the-badge&logo=postgresql" />
  <img src="https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker" />
  <img src="https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws" />
</p>

### 🛡 Engineering
<p align="center">
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript" />
  <img src="https://img.shields.io/badge/JWT-black?style=for-the-badge&logo=jsonwebtokens" />
  <img src="https://img.shields.io/badge/Swagger-85EA2D?style=for-the-badge&logo=swagger" />
  <img src="https://img.shields.io/badge/Clean_Architecture-111111?style=for-the-badge" />
</p>

---

## 📊 GitHub Activity

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=davi-p-oliveira-11&show_icons=true&theme=dracula" />
</p>
